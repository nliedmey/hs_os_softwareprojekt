...
@Route("ui/eventVotingView_Untern")
@Secured("ROLE_UNTERNEHMEN")
public class EventVotingView_Untern extends VerticalLayout { // VerticalLayout fuehrt zu Anordnung von Elementen
	...
	public EventVotingView_Untern(IShowEventService iShowEventService, IShowUnternehmenService iShowUnternehmenService,
		...
		if (vorauswaehltesEvent != null) { // Vereinfachung moeglich, Studierende sind ja jetzt bereits in Event und
											// muessen nicht mehr alle geladen werden
			Event selectedEvent = iShowEventService.showEvent(vorauswaehltesEvent.getEvent_id());
			List<Studierender> listofStudierende = iShowStudierenderService.showStudierende(); // alle Studierenden
																								// holen
		...
		comboBox.addValueChangeListener(event -> {
			...
			if (aEvent != null) {
				...
				studierenderGrid.getDataProvider().refreshAll(); // nach dem Aendern des ComboBox-Wertes wird die
		...
		studierenderGrid.setColumns("vorname", "nachname", "email", "ort", "plz", "strasse"); // Spaltenordnung
																								// festlegen
