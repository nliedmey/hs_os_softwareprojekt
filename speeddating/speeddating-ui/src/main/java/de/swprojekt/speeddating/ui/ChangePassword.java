...
@Secured({"ROLE_EVENTORGANISATOR", "ROLE_STUDENT", "ROLE_UNTERNEHMEN","ROLE_ADMIN"}) //Es muss ein eingeloggter User die Seite aufrufen
public class ChangePassword extends VerticalLayout {
	...
	public ChangePassword(IRegisterUserService iRegisterUserService) {
		...
		buttonAendern.addClickListener(event -> {
			try {
				if(passwordfieldNeuesPasswort.getValue().equals(passwordfieldNeuesPasswortWdh.getValue())) //wenn Eingaben fuer neues Passwort uebereinstimmen
				{
					CustomUserDetails userDetails=(CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal(); //Usernamen des eingeloggten Nutzers holen
					boolean aendernErfolgreich=iRegisterUserService.changePassword(userDetails.getUsername(), passwordfieldAltesPasswort.getValue(), passwordfieldNeuesPasswort.getValue());
