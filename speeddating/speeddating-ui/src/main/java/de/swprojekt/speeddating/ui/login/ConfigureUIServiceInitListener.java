...
@Component
public class ConfigureUIServiceInitListener implements VaadinServiceInitListener { // Spring registriert Listener global
																					// bei allen UI-Instanzen
	...
	private void beforeEnter(BeforeEnterEvent event) {
		...
		if (!SecurityUtils.isAccessGranted(event.getNavigationTarget())) { // wenn Zugriffsrechte fuer Ziel nicht
																			// gewaehrt
			if (SecurityUtils.isUserLoggedIn()) { // wenn keine Rechte, aber eingeloggt
				...
				event.rerouteTo(Login.class); // wenn nicht eingeloggt und Recht aktuell nicht vorhanden, dann
												// Rueckleitung zu Loginview
			}
		}
	}
}
