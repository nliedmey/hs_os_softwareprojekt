...
public class AddEvent extends VerticalLayout {
	...
	public AddEvent(IAddEventService iAddEventService, IShowStudierendeService iShowStudierendeService,
		...
		GridMultiSelectionModel<Studierender> selectionModelStud; // es sollen mehrere Studierende aus Tabelle
																	// ausgewaehlt werden koennen
		Grid<Unternehmen> unternehmenGrid; // Tabelle mit Unternehmen, welche Event zugeordnet werden sollen
		GridMultiSelectionModel<Unternehmen> selectionModelUnternehmen; // es sollen mehrere Studierende aus Tabelle
																		// ausgewaehlt werden koennen
		...
		studierenderGrid.setSelectionMode(SelectionMode.MULTI); // multiselection bedeutet Auswahl mehrerer Studierender
																// in Tabelle moeglich
		...
		ListDataProvider<Unternehmen> ldpUnternehmen = DataProvider
				.ofCollection(iShowUnternehmenService.showUnternehmen()); // Dataprovider erstellen und Quelle fuer
																			// Unternehmen (via Service aus DB)
																			// festlegen
...
//		v1.add(h1);
		...
		buttonHinzufuegen.addClickListener(event -> {
			try {
				...
				LocalDate startzeitpunktDatum = datepickerStartzeitpunktDatum.getValue(); // Datumswert aus Datepicker
																							// ziehen
				LocalTime startzeitpunktUhrzeit = timepickerStartzeitpunktUhrzeit.getValue(); // Zeitwert aus Timepicker
																								// ziehen
				...
				LocalDateTime startzeitpunktldt = LocalDateTime.of(startzeitpunktDatum, startzeitpunktUhrzeit); // zusammenfuegen
																												// von
																												// Date
																												// und
																												// Time
																												// zu
																												// DateTime
				Date startzeitpunkt = Date.from(startzeitpunktldt.atZone(ZoneId.systemDefault()).toInstant()); // Umwandlung
																												// DateTime
																												// in
																												// Date,
																												// weil
				...
				if (endzeitpunkt.getTime() < startzeitpunkt.getTime()) {
					...
					for (Studierender einStudierender : selectionModelStud.getSelectedItems()) { // IDs der
																									// teilnehmenden
																									// Studierenden aus
																									// Tabelle
																									// einsammeln
					...
					for (Unternehmen einUnternehmen : selectionModelUnternehmen.getSelectedItems()) { // IDs der
																										// teilnehmenden
																										// Studierenden
																										// aus Tabelle
																										// einsammeln
					...
					Event erstelltesEvent = iAddEventService.speicherEvent(einEvent); // Uebergabe an Service zur
																						// Speicherung in DB

					CustomUserDetails userDetails = (CustomUserDetails) SecurityContextHolder.getContext()
							.getAuthentication().getPrincipal();

					iAddEventService.addeEventInEventorga(erstelltesEvent.getEvent_id(), userDetails.getEntityRefId()); // dem
																														// Eventorganisator
																														// dieses
																														// Event
																														// zuordnen
