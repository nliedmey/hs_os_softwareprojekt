...
public class AlterEventForEventorganisator extends VerticalLayout {
	...
	public AlterEventForEventorganisator(IShowEventService iShowEventService,
			IShowStudierendeService iShowStudierendeService, IShowUnternehmenService iShowUnternehmenService,
			IAlterEventService iAlterEventService) {
		...
		Label labelAbbruchsuccess = new Label("Eventbearbeitung abgebrochen! ");
...
//		listOfEvents.addAll(iShowEventService.showEvents());
		...
		eventGrid.addSelectionListener(event -> {
			if (!selectionModelEvent.getFirstSelectedItem().isEmpty()) {
				...
				datepickerStartzeitpunktDatum.setValue(zuAendernderndesEvent.getStartzeitpunkt().toInstant()
						.atZone(ZoneId.systemDefault()).toLocalDate());
				timepickerStartzeitpunktUhrzeit.setValue(zuAendernderndesEvent.getStartzeitpunkt().toInstant()
						.atZone(ZoneId.systemDefault()).toLocalTime());
				datepickerEndzeitpunktDatum.setValue(zuAendernderndesEvent.getEndzeitpunkt().toInstant()
						.atZone(ZoneId.systemDefault()).toLocalDate());
				timepickerEndzeitpunktUhrzeit.setValue(zuAendernderndesEvent.getEndzeitpunkt().toInstant()
						.atZone(ZoneId.systemDefault()).toLocalTime());
				...
				Collection<Integer> listStudentenInUnveraendertemEvent = new ArrayList<>(
						iShowEventService.showEvent(zuAendernderndesEvent.getEvent_id()).getTeilnehmendeStudierende());
				Collection<Integer> listUnternehmenInUnveraendertemEvent = new ArrayList<>(
						iShowEventService.showEvent(zuAendernderndesEvent.getEvent_id()).getTeilnehmendeUnternehmen());
				...
				for (Studierender s : ldpStudent.getItems()) {
					if (listStudentenInUnveraendertemEvent.contains(s.getStudent_id())) // wenn Studierender in Event
																						// inkludiert
					{
				...
				for (Unternehmen u : ldpUnternehmen.getItems()) {
					if (listUnternehmenInUnveraendertemEvent.contains(u.getUnternehmen_id())) // wenn Unternehmen in
																								// Event inkludiert
					{
		...
		binder.forField(textfieldRundendauerInMinuten)
				.withConverter(new StringToIntegerConverter("Dauer muss numerisch sein"))
				.asRequired("Dauer darf nicht leer sein...").bind("rundendauerInMinuten");
		...
		aendernButton.addClickListener(event -> {
			...
			if (!textfieldRundendauerInMinuten.getValue()
					.equals(String.valueOf(veraendertesEventDAO.getRundendauerInMinuten()))) {
			...
			if ((!datepickerStartzeitpunktDatum.getValue().equals(
					veraendertesEventDAO.getStartzeitpunkt().toInstant().atZone(ZoneId.systemDefault()).toLocalDate()))
					|| (!timepickerStartzeitpunktUhrzeit.getValue().equals(veraendertesEventDAO.getStartzeitpunkt()
							.toInstant().atZone(ZoneId.systemDefault()).toLocalTime()))) {
				veraendertesEventDAO.setStartzeitpunkt(Date.from(LocalDateTime
						.of(datepickerStartzeitpunktDatum.getValue(), timepickerStartzeitpunktUhrzeit.getValue())
						.atZone(ZoneId.systemDefault()).toInstant()));
			}
			// wenn Datepicker oder Timepicker fuer Endzeitpunkt veraendert
			if ((!datepickerEndzeitpunktDatum.getValue().equals(
					veraendertesEventDAO.getEndzeitpunkt().toInstant().atZone(ZoneId.systemDefault()).toLocalDate()))
					|| (!timepickerEndzeitpunktUhrzeit.getValue().equals(veraendertesEventDAO.getEndzeitpunkt()
							.toInstant().atZone(ZoneId.systemDefault()).toLocalTime()))) {
				veraendertesEventDAO.setEndzeitpunkt(Date.from(LocalDateTime
			...
			Set<Integer> studentenInUnveraendertemEvent = new HashSet<>(
					veraendertesEventDAO.getTeilnehmendeStudierende());
			Set<Integer> studentenInVeraendertemEvent = new HashSet<>();

			Set<Integer> unternehmenInUnveraendertemEvent = new HashSet<>(
					veraendertesEventDAO.getTeilnehmendeUnternehmen());
			...
			if (veraendertesEventDAO.getEndzeitpunkt().getTime() < veraendertesEventDAO.getStartzeitpunkt().getTime()) {
				...
				aendernButton.getUI().ifPresent(ui -> ui.navigate("ui/eventorganisator/menue")); // zurueck auf andere
																									// Seite
			}
		});
